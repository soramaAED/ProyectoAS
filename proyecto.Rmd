...
```{r}
RGB_train_mean[RGB_train_mean$Clase=="soleado",c("R","G","B")]
```

```{r}
RGB_train_mean[RGB_train_mean$Clase=="nublado",c("R","G","B")]
```


```{r}
RGB_train_mean[RGB_train_mean$Clase=="noche",c("R","G","B")]
```
```{r}

boxplot_car<-function(DF,cols){
DF_long <-DF %>%
  pivot_longer(cols = cols, names_to = "Car", values_to = "Value")
...
```{r}
boxplot_car(Car_train_df,c("skew_R","skew_G","skew_B"))
boxplot_car(Car_train_df,c("kurtosis_R", "kurtosis_G", "kurtosis_B"))
boxplot_car(Car_train_df,c("brillo_medio","contraste"))
boxplot_car(Car_train_df,c("porc_claros", "porc_oscuros"))
boxplot_car(Car_train_df,c("gris", "oscuro", "claro"))
```
...
```{r}
boxplot_car(HSV_train_df,c("H","S","V"))
```
...
# boxplot_car(Freq_train_df,c("Baja_Frecuencia"))
# boxplot_car(Freq_train_df,c("Alta_Frecuencia"))
```
...
```{r}
require(fields)
foto<-imagenes_train[[which(meta_train$etiqueta == "soleado")[15]]]
...
CM_XGB <- confusionMatrix(as.factor(predicciones_xgb), as.factor(y_test))
CM_KNN<- confusionMatrix(as.factor(predicciones_knn), as.factor(y_test))
CM_RF <- confusionMatrix(as.factor(predicciones_rf), as.factor(y_test))
CM_NB <- confusionMatrix(as.factor(predicciones_nb), as.factor(y_test))
CM_LOG <- confusionMatrix(as.factor(predicciones_logistico), as.factor(y_test))
```

```{r}
cat("\nMatriz de correlación del modelo SVR:\n")
CM_SVR$table
