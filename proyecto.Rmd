...
```{r}
original_reescalada<-function(img){
...
```{r}
original_reescalada(files_test[12])
```

```{r}
original_reescalada(files_train[130])
```
...
imagen <- imagenes_train[[which(pchSymb_train == 0)[25]]]
...
# Cargar la imagen
imagen <- imagenes_train[[1]]

# Dimensiones deseadas para el reescalado
new_width <- 640
new_height <- 640

# Convertir los tres canales RGB de la imagen
r <- imagen[,,1]  # Canal Rojo
g <- imagen[,,2]  # Canal Verde
b <- imagen[,,3]  # Canal Azul

# Convertir cada canal en imágenes individuales (usando magick)
r_img <- image_blank(width = ncol(r), height = nrow(r), color = "white") %>%
  image_fill(r, gravity = "center")

g_img <- image_blank(width = ncol(g), height = nrow(g), color = "white") %>%
  image_fill(g, gravity = "center")

b_img <- image_blank(width = ncol(b), height = nrow(b), color = "white") %>%
  image_fill(b, gravity = "center")

# Reescalar cada canal individualmente
r_resized <- image_resize(r_img, paste0(new_width, "x", new_height))
g_resized <- image_resize(g_img, paste0(new_width, "x", new_height))
b_resized <- image_resize(b_img, paste0(new_width, "x", new_height))

# Combinar los canales reescalados en una imagen RGB final
imagen_resized <- image_composite(r_resized, g_resized, operator = "over")
imagen_resized <- image_composite(imagen_resized, b_resized, operator = "over")

# Graficar la imagen original y los tres canales reescalados
par(mfrow=c(1,4), mar=c(3,3,3,1))  # Configuración para mostrar 4 gráficos en una fila

# Imagen original
plot(imagen, main="Imagen Original")

# Canal rojo reescalado
plot(as.raster(r_resized), main="Canal Rojo", axes = FALSE)

# Canal verde reescalado
plot(as.raster(g_resized), main="Canal Verde", axes = FALSE)

# Canal azul reescalado
plot(as.raster(b_resized), main="Canal Azul", axes = FALSE)

# Restaurar la configuración de gráficos a 
...
modelo_svr <- svm(X_train, y = y_train, type = "C-classification", kernel = "radial", cost = 10, gamma = 0.1)
...
modelo_rf <- randomForest(x = X_train, y = y_train, ntree = 100, mtry = 5, importance = TRUE)
...
modelo_logistico <- multinom(y_train ~ ., data = X_train)
predicciones_logistico <- predict(modelo_logistico, X_test)
