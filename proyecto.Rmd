...
## Visualización de la reescalada

Visualización de una imagen reescalada por clase.
```{r}
...
### Media de las características RGB
```{r}
...
## Otras características: Asimetría, Kurtosis, contraste, brillo...
...
CM_SVR$table
```
## Visualización de imagenes mal etiquetadas
...
```{r}
CM <- confusionMatrix(as.factor(predicciones_logistico), as.factor(y_test))
CM$table
```
...
```{r}
indices_error <- which(as.factor(predicciones_logistico) != as.factor(y_test))

errores <- data.frame(
  Indice = indices_error,
  Correcto = y_test[indices_error],
  Predicho = predicciones_logistico[indices_error]
)
print(errores)
