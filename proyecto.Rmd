...
tsne_result  <- Rtsne(as.matrix(X_train_estandarizar), dims = 3, perplexity = 30, verbose = TRUE, max_iter = 500)
...
```{r}
CM_SVR <- confusionMatrix(as.factor(predicciones_rf), as.factor(y_test))
CM_SVR$table
```

```{r}
indices_error <- which(as.factor(predicciones_rf) != as.factor(y_test))

errores <- data.frame(
  Indice = indices_error,
  Correcto = y_test[indices_error],
  Predicho = predicciones_rf[indices_error]
)
print(errores)
...
# pca_train_5d <- data.frame(PC1 = pca_train[, 1], PC2 = pca_train[, 2],PC3 = pca_train[, 3] ,PC4 = pca_train[, 4],PC5 = pca_train[, 5])


pca_test<-data.frame(predict(pca_pre_train,newdata=X_test_estandarizar)[,1:7])

# pca_pre_test <- prcomp(scale(as.matrix(X_test_estandarizar)), center = TRUE, scale. = TRUE)
# pca_test <- pca_pre_test$x
# pca_test_5d <- data.frame(PC1 = pca_test[, 1], PC2 = pca_test[, 2], PC3 = pca_test[, 3], PC4 = pca_test[, 4],PC5 = pca_test[, 5])
```
