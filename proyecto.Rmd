...
extraer_caracteristicas_wavelet_detalladas <- function(imagenes_train) {
  ...
  for (j in 1:length(imagenes_train)) {
    ...
    if (!all(dim(img) == c(640, 640))) {
    img <- resize(as.cimg(img), 640, 640)
    }
    ...
    # Extraer sub-bandas
    # Acceder a las sub-bandas del primer nivel
    LL <- slot(wavelet_img, "LL")
    LH <- slot(wavelet_img, "LH")
    HL <- slot(wavelet_img, "HL")
    HH <- slot(wavelet_img, "HH")
